...
```
- 若端点本身已含 `/v1`，则直接用 `<端点>/models`
- 成功的返回应包含 `"object":"list"` 和模型列表
- 从返回结果中筛选出类型为图像生成的模型，记下其 ID

### 3.3 安全保存密钥
使用 Lobe 凭证系统，**绝不在对话中明文暴露密钥**：
```
保存为 kv-env 类型凭证
```
...
lh provider create \
  ...
  -d "<描述>"
```
注意：`--id` 只接受小写字母、数字和连字符。

### 3.5 配置供应商
```bash
lh provider config <providerId> \
  --api-key "<密钥>" \
  --base-url "<端点>" \
  --check-model <用于连通检测的模型ID>
```
...
| 症状 | 可能原因 | 检查方法 |
|------|---------|---------|
| `InvalidProviderAPIKey` | 密钥错误或未配置 | `lh provider config <id> --show` 查看 |
| `ServerError` / 522 | API 后端宕机 | `curl` 直接访问端点 |
| 提交成功但轮询超时 | 异步任务卡住 | 3 分钟后报告用户，建议稍后重试 |
| 模型不显示 | 供应商未启用或模型列表未刷新 | `lh provider toggle --enable` + 重试 |
| `ProviderBizError` | 模型不支持该操作 | 换一个模型或检查 API 文档 |
...
以下模型已在实际使用中测试通过，其余模型理论上兼容但未经验证：
...
> 本技能的方法论适用于所有 OpenAI SDK 兼容的图像生成 API。如果你使用了其他模型并测试成功，欢迎反馈。
